...
import type { DesktopLifecycleState } from '../../shared/desktop-bridge'
...
export function App(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const applyState = (state: DesktopLifecycleState): void => {
      ...
      if (state.status !== 'recoveryRequired' && state.status !== 'failed') {
  ...
  if (lifecycle.status === 'recoveryRequired' || lifecycle.status === 'failed') {
    ...
    return (
      <main className="lifecycle-shell lifecycle-recovery">
        ...
        {diagnosticPreview ? <DiagnosticPreview preview={diagnosticPreview} /> : null}
      </main>
    )
  }
