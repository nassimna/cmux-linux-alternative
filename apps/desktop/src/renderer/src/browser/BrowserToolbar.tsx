...
export function BrowserToolbar({
  ...
  const [draft, setDraft] = useState(() => ({ sourceUrl: state.url, value: state.url }))
  const [validation, setValidation] = useState<{
    sourceUrl: string
    message: string
  } | null>(null)
  const errorId = useId()
  const address = draft.sourceUrl === state.url ? draft.value : state.url
  ...
  const navigate = (): void => {
    ...
    setDraft({ sourceUrl: state.url, value: normalized.url })
    setValidation(null)
    ...
    <div className="browser-toolbar" role="toolbar" aria-label={messages.toolbar.label}>
      ...
      <div className={`browser-address${error ? ' invalid' : ''}`}>
        ...
        <input
          ...
          onChange={(event) => {
            setDraft({ sourceUrl: state.url, value: event.currentTarget.value })
          ...
          onKeyDown={(event) => {
            if (event.key === 'Enter') {
              ...
              setDraft({ sourceUrl: state.url, value: state.url })
              setValidation(null)
